...
namespace old {
	struct Sales_data {
		std::string bookNo;
		void combine(const Sales_data& theData);
		std::string isbn();
	...
	void Sales_data::combine(const Sales_data& theData)
	{
	...
	std::istream& read(std::istream& in, old::Sales_data& theData)
	{
		double price = 0;
		in >> theData.bookNo >> theData.units_sold >> price;
		theData.revenue = price*theData.units_sold;
		return in;
	}
